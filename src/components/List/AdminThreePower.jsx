...
import "./AdminThreePower.css";

const AdminThreePower = ({ exhibitorData }) => {
  ...
  useEffect(() => {
  if (!companies.length || !exhibitorData.length) return;

  const stallMap = {};

  exhibitorData.forEach((ex) => {
    const name = ex.company_name?.trim().toLowerCase();
    if (name) {
      stallMap[name] = ex.stall_no || "";
    }
  });

  const sorted = [...companies].sort((a, b) => {
    const stallA = stallMap[a.toLowerCase()] || "";
    const stallB = stallMap[b.toLowerCase()] || "";

    const parse = (stall) => {
      const match = stall.match(/^([A-Z])-(\d+)/i);
      if (!match) return ["Z", 9999];

      return [match[1].toUpperCase(), parseInt(match[2], 10)];
    };

    const [lA, nA] = parse(stallA);
    const [lB, nB] = parse(stallB);
  ...
  return (
    <div className="admin-power-wrapper">
      <div className="power-top-bar">
        ...
        <div className="search-bar-only-power">
          <input
            type="text"
      ...
      {companies
        .filter((company) => {
          ...
          const stallNo = exhibitor?.stall_no || "";

          const matchCompany = company
            .toLowerCase()
            .includes(searchTerm.toLowerCase());

          const matchStall = stallNo
            .toLowerCase()
            .includes(stallSearch.toLowerCase());

          return matchCompany && matchStall;
        })
        .map((company, index) => {
          ...
          return (
            <div key={index} className="power-accordion">
              ...
              {isOpen && (
                <div className="power-accordion-body-power">
      ...
      {showEditModal && editingRow && (
        <div className="power-modal-overlay">
          <div className="power-modal">
            ...
            <div className="phase-radio-power">
              <label>
                <input
                  type="radio"
                  value="Single"
          ...
          <div className="power-modal large">
            ...
            <div className="exhibitor-list">
              {exhibitorData
                ...
                .map((ex, i) => (
                  <div key={i} className="exhibitor-row-power">
      ...
      {showAddPowerModal && (
        <div className="power-modal-overlay">
          <div className="power-modal">
            ...
            {/* 🔥 EXHIBITION DAYS */}
            <div className="power-box">
              <h4 style={{ marginBottom: "10px", color: "#16a34a" }}>
                Exhibition Days
              </h4>
              ...
              <div className="phase-radio-power-power">
                <label>
                  <input
                    type="radio"
...
export default AdminThreePower;
